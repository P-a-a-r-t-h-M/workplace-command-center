...
function EmailStudio({ onGenerated }: { onGenerated: () => void }) {
  const [input, setInput] = useStoredState<EmailInput>("dayflow-email-input-v2", { recipient: "Jordan Lee, Head of Operations", context: "Cross-functional Q4 launch team", situation: "Legal review is running later than expected", objective: "Confirm the launch timeline and align the team", purpose: "Confirm the timeline for the Q4 launch", keyPoints: "Design handoff is complete\nLegal review is still pending\nThe team needs three business days for QA", action: "confirm whether Friday, October 2 remains achievable", deadline: "2026-09-25", tone: "Friendly" });
  ...
  const apply = (mode: "improve" | "shorter" | "persuasive" | "expand" | "professional") => { setDraft((current) => transformEmail(current, mode)); toast.success(mode === "shorter" ? "Email made more concise" : mode === "expand" ? "Email expanded" : mode === "professional" ? "Professional tone applied" : mode === "persuasive" ? "Persuasive framing added" : "Draft improved"); };
  ...
  return <div className="animate-enter"><PageIntro eyebrow="Write with confidence" title="Smart Email Studio" description="Give the assistant the facts. Get a specific, polished draft you can shape before sending." />
    <div className="grid gap-6 xl:grid-cols-[0.86fr_1.14fr]">
       <div className="space-y-5"><section className="glass-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><h2 className="panel-title">Email details</h2><Badge variant="outline" className="font-medium text-muted-foreground">Personalized brief</Badge></div><div className="space-y-4"><Field label="Recipient"><Input value={input.recipient} onChange={(e) => update("recipient", e.target.value)} placeholder="Who is this for?" /></Field><Field label="Context"><Input value={input.context} onChange={(e) => update("context", e.target.value)} placeholder="Relationship, team, or background" /></Field><Field label="Situation"><Textarea className="min-h-20" value={input.situation} onChange={(e) => update("situation", e.target.value)} placeholder="What is happening?" /></Field><Field label="Objective"><Input value={input.objective} onChange={(e) => update("objective", e.target.value)} placeholder="What outcome do you want?" /></Field><Field label="Purpose"><Input value={input.purpose} onChange={(e) => update("purpose", e.target.value)} placeholder="What should this email accomplish?" /></Field><Field label="Key points"><Textarea className="min-h-28" value={input.keyPoints} onChange={(e) => update("keyPoints", e.target.value)} placeholder="One point per line" /></Field><Field label="Desired action"><Input value={input.action} onChange={(e) => update("action", e.target.value)} placeholder="What should they do next?" /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Deadline (optional)"><Input type="date" value={input.deadline} onChange={(e) => update("deadline", e.target.value)} /></Field><Field label="Tone"><Select value={input.tone} onValueChange={(value) => update("tone", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Formal">Formal</SelectItem><SelectItem value="Friendly">Friendly</SelectItem><SelectItem value="Persuasive">Persuasive</SelectItem></SelectContent></Select></Field></div><Button className="h-11 w-full rounded-xl" onClick={generate}><Wand2 /> Generate email</Button></div></section>
      ...
      <section className="editor-shell"><div className="editor-toolbar"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Editable draft</p><p className="mt-1 text-xs text-muted-foreground">{words} words · {draft.body.length} characters</p></div><div className="flex flex-wrap justify-end gap-1"><Button variant="ghost" size="sm" onClick={() => apply("improve")}><Sparkle /> Improve</Button><Button variant="ghost" size="sm" onClick={() => apply("shorter")}><FilePenLine /> Shorten</Button><Button variant="ghost" size="sm" onClick={() => apply("expand")}><Plus /> Expand</Button><Button variant="ghost" size="sm" onClick={() => apply("professional")}><Pencil /> Professional</Button><Button variant="ghost" size="sm" onClick={() => apply("persuasive")}><Target /> Persuasive</Button></div></div><div className="p-5 sm:p-7"><Label htmlFor="subject" className="text-xs uppercase tracking-[0.12em] text-muted-foreground">Subject</Label><Input id="subject" className="mt-2 h-auto border-0 bg-transparent px-0 py-2 font-display text-xl font-bold shadow-none focus-visible:ring-0" value={draft.subject} onChange={(e) => setDraft({ ...draft, subject: e.target.value })} /><div className="my-4 h-px bg-border" /><Textarea aria-label="Email body" className="min-h-[540px] resize-none border-0 bg-transparent p-0 text-[15px] leading-7 shadow-none focus-visible:ring-0" value={draft.body} onChange={(e) => setDraft({ ...draft, body: e.target.value })} /></div><div className="editor-footer"><Button variant="outline" onClick={generate}><RefreshCw /> Regenerate</Button><Button onClick={() => { navigator.clipboard.writeText(`${draft.subject}\n\n${draft.body}`); toast.success("Email copied to clipboard"); }}><Copy /> Copy email</Button></div></section>
    </div>
  </div>;
}
...
function TaskPlanner({ onPlanned }: { onPlanned: (count: number) => void }) {
  ...
  return <div className="animate-enter"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><PageIntro eyebrow="Protect your attention" title="AI Task Planner" description="Balance urgency, impact, and effort into a schedule you can actually finish." /><div className="segmented"><button className={mode === "daily" ? "active" : ""} onClick={() => setMode("daily")}>Daily</button><button className={mode === "weekly" ? "active" : ""} onClick={() => setMode("weekly")}>Weekly</button></div></div>
    <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]"><div className="space-y-5"><section className="glass-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><h2 className="panel-title">Add priorities</h2><span className="text-xs text-muted-foreground">{tasks.length} tasks</span></div><div className="space-y-4"><Field label="Task"><Input value={newTask.title} onChange={(e) => setNewTask({ ...newTask, title: e.target.value })} placeholder="e.g. Prepare client proposal" /></Field><div className="grid grid-cols-2 gap-3"><Field label="Deadline"><Input type="date" value={newTask.deadline} onChange={(e) => setNewTask({ ...newTask, deadline: e.target.value })} /></Field><Field label="Duration"><Select value={String(newTask.duration)} onValueChange={(value) => setNewTask({ ...newTask, duration: Number(value) })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{[30,45,60,90,120].map((duration) => <SelectItem key={duration} value={String(duration)}>{duration} min</SelectItem>)}</SelectContent></Select></Field></div><div className="grid grid-cols-2 gap-3"><Field label="Importance"><Select value={newTask.importance} onValueChange={(value) => setNewTask({ ...newTask, importance: value as PlannerTask["importance"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["Low","Medium","High"].map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></Field><Field label="Urgency"><Select value={newTask.urgency} onValueChange={(value) => setNewTask({ ...newTask, urgency: value as PlannerTask["urgency"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["Low","Medium","High"].map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></Field></div><Button variant="outline" className="w-full" onClick={addTask}><Plus /> Add task</Button></div><div className="mt-5 space-y-2 border-t border-border pt-4">{tasks.map((task) => <div key={task.id} className="flex items-center gap-3 rounded-lg bg-muted/60 p-3"><div className={cn("size-2 rounded-full", task.urgency === "High" ? "bg-destructive" : "bg-primary")} /><p className="min-w-0 flex-1 truncate text-sm font-medium">{task.title}</p><Button variant="ghost" size="icon-sm" aria-label={`Delete ${task.title}`} onClick={() => setTasks(tasks.filter((item) => item.id !== task.id))}><Trash2 /></Button></div>)}</div></section><Button className="h-12 w-full rounded-lg" onClick={replan}><RefreshCw /> Replan my day</Button></div>
       <section className="schedule-shell"><div className="schedule-head"><div><p className="eyebrow">{mode === "daily" ? "Wednesday, September 23" : "September 21–25"}</p><h2 className="panel-title mt-1">Your optimized {mode === "daily" ? "day" : "week"}</h2></div><div className="flex flex-wrap items-center justify-end gap-2"><Button size="sm" variant="outline" onClick={() => { setSchedule(planTasks(tasks)); toast.success("Tasks prioritized"); }}><Target /> Prioritize</Button><Button size="sm" variant="outline" onClick={() => setMode("daily")}>Plan My Day</Button><Button size="sm" variant="outline" onClick={() => setMode("weekly")}>Plan My Week</Button><div className="ml-2 text-right"><p className="text-lg font-bold">{Math.floor(total / 60)}h {total % 60}m</p><p className="text-[11px] text-muted-foreground">Focus planned</p></div></div></div><div className="p-4 sm:p-6">{mode === "daily" ? <Timeline schedule={schedule} /> : <Weekly schedule={schedule} />}</div><div className="priority-note"><div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Gauge /></div><p className="text-sm leading-6"><strong>Why this order?</strong> High-impact work with the nearest deadline is protected first, while shorter coordination tasks are grouped later to reduce context switching.</p></div></section>
    </div>
  </div>;
}
...
function WorkplaceAssistant({ onMessage }: { onMessage: () => void }) {
  const [savedMessages, setSavedMessages] = useStoredState<UIMessage[]>("dayflow-chat-v2", initialMessages);
  const transport = useMemo(() => new DefaultChatTransport({ api: "/api/chat" }), []);
  const { messages, sendMessage, regenerate, status, error, setMessages } = useChat({ id: "dayflow-main-chat", messages: savedMessages, transport, onFinish: ({ messages: complete }) => { setSavedMessages(complete); onMessage(); }, onError: (chatError) => toast.error(chatError.message) });
  ...
  const send = async (text: string) => { const prompt = text.trim(); if (!prompt || thinking) return; await sendMessage({ text: prompt }); };
  const transform = (message: UIMessage, instruction: string) => { const text = messageText(message); if (text) send(`${instruction}\n\n${text}`); };
  return <div className="animate-enter"><div className="flex items-end justify-between"><PageIntro eyebrow="Your thinking partner" title="AI Workplace Assistant" description="Turn messy workplace inputs into clear messages, plans, and decisions." /><Button variant="ghost" size="sm" onClick={() => { setMessages(initialMessages); setSavedMessages(initialMessages); toast.success("Conversation cleared"); }}><RotateCcw /> New conversation</Button></div><div className="assistant-shell"><div className="assistant-context"><div className="flex items-center gap-3"><div className="ai-mark"><Bot /></div><div><h2 className="font-display font-bold">Dayflow Assistant</h2><p className="text-xs text-muted-foreground">AI-powered · saved in this browser</p></div></div><div className="mt-6"><p className="mb-3 text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Try asking</p><div className="flex flex-wrap gap-2 xl:flex-col">{suggestions.map((suggestion) => <Button key={suggestion} variant="outline" className="h-auto justify-start whitespace-normal py-2.5 text-left text-xs" onClick={() => send(suggestion)}>{suggestion}<ChevronRight className="ml-auto" /></Button>)}</div></div><div className="mt-auto hidden rounded-2xl bg-muted p-4 text-xs leading-5 text-muted-foreground xl:block"><strong className="text-foreground">Tip:</strong> Include the audience, outcome, constraints, and tone for a sharper response.</div></div><div className="flex min-h-[650px] min-w-0 flex-col"><Conversation className="min-h-0"><ConversationContent className="mx-auto w-full max-w-3xl gap-7 px-4 py-8 sm:px-8">{messages.map((message) => <Message key={message.id} from={message.role}><MessageContent className={message.role === "user" ? "bg-chat-user text-chat-user-foreground" : ""}>{message.role === "assistant" && <span className="mb-1 flex items-center gap-2 text-xs font-bold text-primary"><Bot className="size-4" /> DAYFLOW AI</span>}<MessageResponse>{messageText(message)}</MessageResponse></MessageContent>{message.role === "assistant" && message.id !== "welcome" && <MessageActions className="flex-wrap"><MessageAction tooltip="Copy response" onClick={() => { navigator.clipboard.writeText(messageText(message)); toast.success("Response copied"); }}><Copy /></MessageAction><MessageAction tooltip="Regenerate" onClick={() => regenerate({ messageId: message.id })}><RefreshCw /></MessageAction><Button size="sm" variant="ghost" onClick={() => transform(message, "Make this response shorter while preserving key details.")}>Shorter</Button><Button size="sm" variant="ghost" onClick={() => transform(message, "Rewrite this in a more professional tone.")}>Professional</Button><Button size="sm" variant="ghost" onClick={() => transform(message, "Turn this into a complete professional email with a subject line.")}>Turn into Email</Button><Button size="sm" variant="ghost" onClick={() => transform(message, "Turn this into a prioritized task list with clear next actions.")}>Turn into Tasks</Button><Button size="sm" variant="ghost" onClick={() => transform(message, "Extract action items, owners, and deadlines from this.")}>Action Items</Button></MessageActions>}</Message>)}{thinking && <Message from="assistant"><MessageContent><Shimmer>Thinking through your request...</Shimmer></MessageContent></Message>}{error && <p className="chat-error">{error.message}</p>}</ConversationContent><ConversationScrollButton /></Conversation><div className="border-t border-border bg-background/70 p-4 sm:p-6"><div className="mx-auto max-w-3xl"><PromptInput onSubmit={(message) => send(message.text || "")} className="rounded-2xl border-border bg-card shadow-editor"><PromptInputTextarea autoFocus placeholder="Ask for a message, summary, agenda, ideas, or prioritization help…" className="min-h-24" /><PromptInputFooter className="justify-between"><span className="pl-2 text-[11px] text-muted-foreground">Enter to send · Shift+Enter for new line</span><PromptInputSubmit status={thinking ? "submitted" : "ready"} disabled={thinking} /></PromptInputFooter></PromptInput></div></div></div></div></div>;
}
